...
export const useBookShelfGsap = () => {
  ...
  useLayoutEffect(() => {
    if (isMobile || !bookShelfContainerRef.current) {
      return
    }

    const trigger = ScrollTrigger.create({
      id: 'book-shelf-trigger',
      trigger: bookShelfContainerRef.current,
      start: 'top 120px',
      pin: false,
      pinSpacing: false,
      scrub: false,
  ...
  useLayoutEffect(() => {
    ...
    const books = Array.from(
      track.querySelectorAll('[data-book-container="true"]'),
    ) as HTMLElement[]
    ...
    const getBookPosition = (index: number) => {
      ...
      for (let i = 0; i < index; i++) {
        const book = books[i]
        const bookWidth =
          parseFloat(book.style.getPropertyValue('--w')) || 100
        const marginLeft =
          parseFloat(book.style.getPropertyValue('--book-margin-left')) || 0
        const marginRight =
          parseFloat(book.style.getPropertyValue('--book-margin-right')) || 0
    ...
    const observer = Observer.create({
      target: bookShelfContainerRef.current,
