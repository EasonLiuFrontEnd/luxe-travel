...
type TFreeToursBannerProps = TBaseComponent & {
  tours?: Array<{
    id: string
    title: string
    subtitle: string
...
const FreeToursBanner = ({ className, tours = [] }: TFreeToursBannerProps) => {
  ...
  const currentContent = useApiData
    ? {
        title: tours[currentSlide - 1]?.title || '',
        subtitle: tours[currentSlide - 1]?.subtitle || '',
        description: tours[currentSlide - 1]?.description || ''
      }
    ...
    >
      ...
      <div className='xl:relative flex flex-col rounded-2xl xl:w-full xl:h-[670px]'>
        <div className='relative xl:absolute xl:inset-0 flex w-full h-[460px] xl:h-full'>
          <Image
            src={currentImageUrl || `/free-tours/${currentSlide}.jpg`}
            alt={`自由行程精選 ${currentSlide}`}
            width={1824}
            height={670}
...
export default FreeToursBanner