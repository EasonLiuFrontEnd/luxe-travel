...
const DestinationFilter = ({ className, onSearch }: TDestinationFilterProps) => {
  ...
  const getSelectedCountryNames = () => {
    ...
    return countryNames.slice(0, 2).join('、') + (countryNames.length > 2 ? `等${countryNames.length}個國家` : '')
  }
  ...
  return (
    ...
    >
      {/* 交錯背景圖層 - 上排和下排 */}
      <div className={`absolute inset-0 opacity-15 ${styles.backgroundLayer1}`} />
      <div className={`absolute inset-0 opacity-15 ${styles.backgroundLayer2}`} />
      <div className={`absolute inset-0 opacity-15 ${styles.backgroundLayer3}`} />
        ...
        <div className='bg-white rounded-2xl p-6 flex flex-col lg:flex-row gap-[20px] lg:gap-9 lg:items-center'>
          <div className='flex-1 relative' ref={dropdownRef}>
            <div className='flex flex-col gap-1'>
              ...
              >
                <span className={`font-family-genseki text-base ${selectedCountries.length > 0 ? 'text-figma-primary-950' : 'text-figma-primary-300'}`}>
